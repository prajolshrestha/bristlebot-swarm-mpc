...
from expert_src.controller import (
    CasadiAcadosEllipticalController, D_SAFE, D_SAFE_OBS,
...
GLOBAL_Q_COH      = float(_CFG_REYNOLDS.get("q_cohesion", Q_COHESION_DEFAULT))
GLOBAL_Q_ALI      = float(_CFG_REYNOLDS.get("q_align", Q_ALIGN_DEFAULT))
...
def _build_obstacle_manager(use_obstacles: "bool | None" = None) -> "ObstacleManager | None":
    ...
    multi-seed driver toggles it via H.ENABLE_OBSTACLES); callers may pass an
...
RESULTS_BASE_DIR = "ablation_results_hard_bf"
...
class HeadlessSim:
    def __init__(self, n_robots=10, behavior_mode=SwarmBehavior.PHOTOTAXIS,
                 q_coh=GLOBAL_Q_COH, q_ali=GLOBAL_Q_ALI,
                 q_pos=GLOBAL_Q_POS, q_theta=GLOBAL_Q_THETA, q_vel=GLOBAL_Q_VEL,
                 q_om=GLOBAL_Q_OM, r_u=GLOBAL_R_U,
