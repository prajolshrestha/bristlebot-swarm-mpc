...
def ellipse_robot_tracking_mpc(constraints: Dict[str, float]):
    ...
    model_name = "ellipse_robot_tracking_mpc_dt_hocbf_off"
    ...
    perp_x2 = -sin(th_next)
    perp_y2 =  cos(th_next)
    off_x2 = if_else(om_cmd > 0, d_ccw * perp_x2, -d_cw * perp_x2)
    off_y2 = if_else(om_cmd > 0, d_ccw * perp_y2, -d_cw * perp_y2)
    px_next2 = px_next + TS * (v_cmd * cos(th_next) + om_cmd * off_y2)
    py_next2 = py_next + TS * (v_cmd * sin(th_next) - om_cmd * off_x2)
