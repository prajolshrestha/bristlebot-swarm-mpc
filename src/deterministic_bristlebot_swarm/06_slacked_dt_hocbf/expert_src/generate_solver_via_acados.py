...
def build_acados_solver(
    ...
    jdata["trajectory_coupled_dmpc"] = {
        ...
        "hocbf_gamma1":       model.HOCBF_GAMMA1,
        "hocbf_gamma2":       model.HOCBF_GAMMA2,
        "hocbf_gamma1_obs":   model.HOCBF_GAMMA1_OBS,
        "hocbf_gamma2_obs":   model.HOCBF_GAMMA2_OBS,
        "Ts":                 model.TS,
        "nh_total":           nh,
        "nh_robots":          2 * K,
        "nh_obs":             2 * M,
        ...
        "constraint_type":    "hybrid: slacked BF + slacked Dt-HOCBF psi2>=0 at stages",
        "slack_Zl_rob":       float(Zl_rob),
        "slack_zl_rob":       float(zl_rob),
        "slack_Zl_obs":       float(Zl_obs),
        "slack_zl_obs":       float(zl_obs),
        "description": (
            "Hybrid slacked BF+Dt-HOCBF: stage h_j>=0 AND psi2>=0 (DISCRETE "
            "exponential chain from one-step differences, psi2 = h(x2) - "
            "(2-g1-g2)h(x1) + (1-g1)(1-g2)h(x0); om authority via th_next in the "
            "two-step Euler prediction), terminal slacked h_j>=0. Distance slack "
            "penalized 8x vs Dt-HOCBF slack. Neighbour/obstacle assumed locally "
            "constant-velocity, ego (v,om) constant over both substeps."
        ),
    }
...
if __name__ == "__main__":
    ...
    hocbf_g1    = float(col.get("hocbf_gamma1", 0.5))
    hocbf_g2    = float(col.get("hocbf_gamma2", 0.5))
    ...
    hocbf_g1_obs    = float(obs.get("hocbf_gamma1_obs", hocbf_g1))
    hocbf_g2_obs    = float(obs.get("hocbf_gamma2_obs", hocbf_g2))
    ...
    print(f"  cbf_alpha (robots)   : {cbf_alpha}  (legacy, unused by Dt-HOCBF stage)")
    print(f"  cbf_alpha (obstacles): {cbf_alpha_obs}  (legacy, unused by Dt-HOCBF stage)")
    print(f"  Dt-HOCBF gamma1/2 rob: {hocbf_g1} / {hocbf_g2}  (discrete decay, (0,1])")
    print(f"  Dt-HOCBF gamma1/2 obs: {hocbf_g1_obs} / {hocbf_g2_obs}")
    ...
    print(f"  Constraint type      : hybrid slacked BF + slacked Dt-HOCBF psi2>=0 at stages")
