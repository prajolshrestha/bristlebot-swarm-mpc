...
def build_acados_solver(
    ...
    if os.name == "nt":
        ...
        cmake_builder.additional_cmake_options = (
            "-DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_OBJECT_PATH_MAX=500"
        )
    ...
    jdata["trajectory_coupled_dmpc"] = {
        ...
        "lookahead_dist":     model.L_LOOKAHEAD,
        "Ts":                 model.TS,
        "nh_total":           nh,
        "nh_robots":          2 * K,
        "nh_obs":             2 * M,
...
if __name__ == "__main__":
    ...
    lookahead   = float(col.get("lookahead_dist", 0.05))
    ...
    print(f"  lookahead_dist L     : {lookahead * 100:.1f} cm  (Dt-CBF handle ahead of robot)")
