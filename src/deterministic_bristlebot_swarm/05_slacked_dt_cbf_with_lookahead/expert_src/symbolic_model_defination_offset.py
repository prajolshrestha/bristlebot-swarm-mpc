...
def ellipse_robot_tracking_mpc(constraints: Dict[str, float]):
    """
    Distributed Predictive Flocking, hybrid v2: slacked barrier anchor + slacked
    discrete-time CBF on a LOOK-AHEAD HANDLE (ω-steerable Dt-CBF).

    Safety formulation
    ------------------
    For each neighbor j and obstacle m, TWO slacked rows per stage:

      (A) BF anchor on the BODY centre, full radius:
            h_j(x_k) = ||p - p_j||² - D_safe²  ≥  0
      (B) Discrete-time CBF on the look-ahead HANDLE, reduced radius:
            h_L(x̂_{k+1}) - (1-α)·h_L(x_k)  ≥  0
          with  h_L(x) = ||pL - p_j||² - D_handle²,
                pL     = p + L·[cos θ, sin θ]          (handle, L = lookahead_dist)
                D_handle = max(D_safe - L, 0.005)

    Why the handle: with the plain barrier h(p), the one-step Euler prediction
    px̂ = px + Ts·v·cos(θ) gives ∂h(x̂)/∂ω = 0 (relative degree 2) — the Dt-CBF
    row can only brake, not steer. The handle's next position uses
    θ_{k+1} = θ + Ts·ω, so ω has non-zero gradient on the barrier and the
    solver can STEER away. The handle radius is reduced by L so the handle
    barrier goes marginal exactly when the BODY reaches D_safe head-on; body
    safety at full D_safe is still guaranteed by row (A) + the terminal barrier.
    The neighbour is advanced by its reported velocity: nb_{k+1} = nb + Ts·nb_v.

    All rows are softened via acados con_h_expr / nsh slack variables for
    guaranteed feasibility. Slack penalties (Zl, zl) are set in
    generate_solver_via_acados.py and can be updated at runtime via
    solver.cost_set(k, 'Zl', ...).

    TERMINAL STAGE: con_h_expr_e enforces only the simple BODY barrier
    h_j(x_N) ≥ 0 (no control input at the terminal stage, so no CBF row).
    ...
    NOTE: cbf_alpha and lookahead_dist are compiled into the solver
    (CBF_ALPHA, CBF_ALPHA_OBS, L_LOOKAHEAD). NP_BASE = 12 unchanged from the
    slack version.
    ...
    ────────────────────────────────────────────────
      Stage (k=0..N-1):
        ...
        Row K..2K-1:       h_L(x̂_{k+1}) - (1-α)·h_L(x_k)  ≥ 0   (handle Dt-CBF, robots)
        Row 2K..2K+M-1:    h_m(x_k)                        ≥ 0   (BF anchor, obstacles)
        Row 2K+M..2(K+M)-1: handle Dt-CBF, obstacles (α_obs, D_handle_obs)
      Terminal (k=N, nh_e = K + M):  h_j(x_N) ≥ 0  and  h_m(x_N) ≥ 0
    """
    model      = types.SimpleNamespace()
    constraint = types.SimpleNamespace()
    model_name = "ellipse_robot_tracking_mpc_lookahead_cbf_off"
    ...
    L_LOOKAHEAD    = float(constraints.get("lookahead_dist", 0.05))
    ...
    pLx      = px      + L_LOOKAHEAD * cos(th)
    pLy      = py      + L_LOOKAHEAD * sin(th)
    pLx_next = px_next + L_LOOKAHEAD * cos(th_next)
    pLy_next = py_next + L_LOOKAHEAD * sin(th_next)
    ...
    D_HANDLE       = fmax(D_SAFE     - L_LOOKAHEAD, 0.005)
    D_HANDLE_OBS   = fmax(D_SAFE_OBS - L_LOOKAHEAD, 0.005)
    D_HANDLE_SQ    = D_HANDLE     ** 2
    D_HANDLE_OBS_SQ = D_HANDLE_OBS ** 2
    BIG       = SX(100.0)
    ...
    for j in range(MAX_NEIGHBOURS):
        dx_jL = pLx - nb_px[j]
        dy_jL = pLy - nb_py[j]
        h_jL  = dx_jL * dx_jL + dy_jL * dy_jL - D_HANDLE_SQ

        nb_px_n = nb_px[j] + TS * nb_vx[j]
        nb_py_n = nb_py[j] + TS * nb_vy[j]
        dx_nL   = pLx_next - nb_px_n
        dy_nL   = pLy_next - nb_py_n
        h_jL_n  = dx_nL * dx_nL + dy_nL * dy_nL - D_HANDLE_SQ

        cbf_j         = h_jL_n - (1.0 - CBF_ALPHA) * h_jL
    ...
    for m in range(MAX_OBS):
        dx_mL = pLx - obs_px[m]
        dy_mL = pLy - obs_py[m]
        h_mL  = dx_mL * dx_mL + dy_mL * dy_mL - D_HANDLE_OBS_SQ

        dx_nL = pLx_next - obs_px[m]
        dy_nL = pLy_next - obs_py[m]
        h_mL_n = dx_nL * dx_nL + dy_nL * dy_nL - D_HANDLE_OBS_SQ

        cbf_m         = h_mL_n - (1.0 - CBF_ALPHA_OBS) * h_mL
