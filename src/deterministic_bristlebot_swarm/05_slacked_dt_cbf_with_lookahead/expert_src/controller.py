...
class CasadiAcadosEllipticalController:
    ...
    def __init__(
        ...
        self.deadlock_bias       = 0.6
        self.deadlock_range      = 3.5 * D_SAFE
        self.deadlock_cos_cone   = 0.5

        self.priority_enable     = True
        self.yield_bias          = 1.8
        self.yield_speed_scale   = 0.5
    ...
    @staticmethod
    def _priority_key(p: np.ndarray, eps: float) -> tuple[float, float, float]:
        """
        Deterministic, communication-free priority for a position p.

        Primary key  = atan2(p_y, p_x): the angular position about the world
        origin. Two robots in a conflict each evaluate this on the SAME two
        positions (their own + the observed neighbour position), so they agree on
        the ordering without exchanging IDs or messages. The tie-break is a
        lexicographic key on coordinates rounded to ~eps, used only when the two
        angles are within eps (near-collinear through the origin), keeping the
        order total and identical on both sides.

        Compared with strict ">", a LARGER key means HIGHER priority (proceeds).
        """
        ang = float(np.arctan2(p[1], p[0]))
        ang_q = round(ang / eps) * eps
        return (ang_q, round(float(p[0]) / eps) * eps,
                       round(float(p[1]) / eps) * eps)

    def _apply_deadlock_bias(
        self,
        robot,
        goal:          np.ndarray,
        ref_theta:     float,
        ...
        """
        Rotate the tracking reference to the robot's RIGHT when a neighbour (or
        obstacle) is close and roughly ahead, so a symmetric head-on standoff
        resolves with a consistent passing side.

        On top of the symmetric "pass-on-the-right" bias, an ASYMMETRIC,
        position-based priority tie-break decides, per pairwise conflict, who
        yields. The higher-priority robot suppresses its bias (drives straight
        through); the lower-priority robot turns wider (yield_bias) AND slows
        (yield_speed_scale) to give way. Both robots compute the same priority
        from observable positions, so they agree without communication.

        Returns possibly-modified (goal, ref_theta, speed_scale). speed_scale is
        1.0 except when this robot is yielding to a higher-priority neighbour.
        The rotation/scaling is transient: it vanishes once nothing is blocking
        the heading, so nominal open-space tracking is unaffected.
        """
        if self.deadlock_bias <= 0.0:
            return goal, ref_theta, 1.0
        ...
        for traj in nb_trajs:
            blockers.append((np.asarray(traj[0][:2], float), True))
        if obstacle_manager is not None:
            try:
                for obs in obstacle_manager.get_nearest_k(pos):
                    blockers.append((np.asarray(obs.pos, float), False))
            except Exception:
                pass
        ...
        higher_ahead = False
        robot_ahead  = False
        pr_self      = self._priority_key(pos, self.priority_eps)
        for b, is_robot in blockers:
            to_b = b - pos
            d    = float(np.linalg.norm(to_b))
            if d < 1e-4 or d > self.deadlock_range:
                continue
            ahead = float(np.dot(to_b / d, heading))
            if ahead < self.deadlock_cos_cone:
                continue
            strength = (1.0 - d / self.deadlock_range) * ahead
            worst = max(worst, strength)
            if is_robot:
                robot_ahead = True
                pr_b = self._priority_key(b, self.priority_eps)
                if pr_b > pr_self:
                    higher_ahead = True
        ...
        speed_scale = 1.0
        if self.priority_enable and robot_ahead:
            if higher_ahead:
                bias        = self.yield_bias
                speed_scale = self.yield_speed_scale
            else:
                bias = 0.0
        else:
            bias = self.deadlock_bias

        if bias <= 0.0:
            return goal, ref_theta, speed_scale

        delta = -bias * worst
        c, s  = np.cos(delta), np.sin(delta)
        R     = np.array([[c, -s], [s, c]])

        to_goal = np.asarray(goal, float) - pos
        dist    = float(np.linalg.norm(to_goal))
        if dist < 1e-4:
            return goal, ref_theta, speed_scale
        dir_eff   = R @ (to_goal / dist)
        goal_eff  = pos + dir_eff * dist
        theta_eff = float(np.arctan2(dir_eff[1], dir_eff[0]))
        return goal_eff, theta_eff, speed_scale
    ...
    def compute_control(
        ...
        goal, ref_theta, yield_speed_scale = self._apply_deadlock_bias(
            robot, goal, ref_theta, nb_trajs_norm, obstacle_manager
        )
        ...
        if status != 0:
            ...
            if yield_speed_scale != 1.0:
                u[0] *= yield_speed_scale
